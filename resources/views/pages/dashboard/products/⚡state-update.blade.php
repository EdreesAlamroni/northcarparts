...
new class extends Component
{
    ...
    public function submit(): void
    {
        ...
        $this->product = $this->product->state->transitionTo(ProductState::resolve($validated['state']));
    ...
    <flux:modal name="state-update" class="md:w-lg w-sm">
        <form wire:submit="submit" class="space-y-6">
            <div>
                ...
                <flux:text class="mt-2">{{ __('اختر الحالة الجديدة للمنتج.') }}</flux:text>
            </div>
            ...
            <div class="space-y-6">
                <flux:field>
                    ...
                    <flux:select id="state" wire:model="state" :placeholder="__('اختر الحالة')" required>
                        @foreach ($this->product->getTransitionableStates() as $option)
