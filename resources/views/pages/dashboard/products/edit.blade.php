<x-layouts::dashboard :title="__('تعديل بيانات المنتج')">
    ...
    <div class="space-y-6">
        ...
        <section>
            <form action="{{ route('dashboard.products.update', $product) }}" method="POST" class="non-wire">
                @csrf
                @method('PUT')
                    ...
                    <x-slot:slot class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <flux:field>
                                ...
                                <flux:input type="text" id="code" name="code" :value="old('code', $product->code)" autocomplete="off" lang="en" required />
                                <flux:error name="code" />
                            </flux:field>

                            <flux:field>
                                <flux:label @class(['text-red-600' => $errors->has('slug')]) for="slug" badge="*" required>{{ __('الرابط') }}</flux:label>
                                <flux:input type="text" id="slug" name="slug" :value="old('slug', $product->slug)" autocomplete="off" lang="en" required />
                                <flux:error name="slug" />
                            </flux:field>

                            <flux:field>
                                <flux:label @class(['text-red-600' => $errors->has('name')]) for="name" badge="*" required>{{ __('الاسم') }}</flux:label>
                                <flux:input type="text" id="name" name="name" :value="old('name', $product->name)" autocomplete="off" required />
                            ...
                            <flux:field>
                                ...
                                <flux:select name="filter_type" id="filter_type" :placeholder="__('اختر نوع الفلتر')">
                                    @foreach ($filterTypes as $type)
                                        <flux:select.option :value="$type->id" :selected="old('filter_type', $product->filter_type->value) === $type->id">
                                            {{ $type->name }}
                                        </flux:select.option>
                                    @endforeach
                                </flux:select>
                            ...
                            <flux:field>
                                ...
                                <flux:input type="url" id="qr_code_redirect_url" name="qr_code_redirect_url" :value="old('qr_code_redirect_url', $product->qr_code_redirect_url)" autocomplete="off" lang="en" required />
                            ...
                            <flux:field>
                                ...
                                <flux:input type="number" id="sort_order" name="sort_order" class="font-mono" :value="old('sort_order', $product->sort_order)" min="1" step="1" lang="en" required />
                            ...
                            <flux:field>
                                ...
                                <flux:select name="state" id="state">
                                    @foreach ($states as $state)    
                                        <flux:select.option :value="$state->id" :selected="old('state', $product->state->value()) === $state->id">
