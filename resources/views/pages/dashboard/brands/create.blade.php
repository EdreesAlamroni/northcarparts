<x-layouts::dashboard :title="__('إضافة علامة تجارية جديدة')">
    @php
        $breadcrumbs = [
            ['name' => __('العلامات التجارية'), 'url' => route('dashboard.brands.index')],
            ['name' => __('إضافة علامة تجارية جديدة'), 'active' => true],
        ];
    @endphp
    ...
    <div class="space-y-6">
        ...
        <section>
            <form action="{{ route('dashboard.brands.store') }}" method="POST" class="non-wire">
                @csrf

                <x-card>
                    <x-slot:heading>
                        <x-slot:title>
                            <flux:heading>{{ __('إضافة علامة تجارية جديدة') }}</flux:heading>
                        </x-slot:title>
                        <x-slot:description>
                    ...
                    <x-slot:footer class="flex items-center justify-end gap-x-3">
                        <flux:button :href="route('dashboard.brands.index')" size="sm" icon="arrow-uturn-left" wire:navigate>{{ __('إلغاء الأمر') }}</flux:button>
