<x-layouts::dashboard :title="__('إضافة خاصية جديدة')">
    @php
        $breadcrumbs = [
            ['name' => __('خصائص المنتجات'), 'url' => route('dashboard.specifications.index')],
            ['name' => __('إضافة خاصية جديدة'), 'active' => true],
        ];
    ...
    <div class="space-y-6">
        ...
        <section>
            <form action="{{ route('dashboard.specifications.store') }}" method="POST" class="non-wire">
                @csrf

                <x-card>
                    <x-slot:heading>
                        <x-slot:title>
                            <flux:heading>{{ __('إضافة خاصية جديدة') }}</flux:heading>
                        </x-slot:title>
                        <x-slot:description>
                    ...
                    <x-slot:slot class="space-y-6">
                        ...
                        @include('pages.dashboard.specifications._values-form', ['values' => $values])
                    </x-slot:slot>

                    <x-slot:footer class="flex items-center justify-end gap-x-3">
                        <flux:button :href="route('dashboard.specifications.index')" size="sm" icon="arrow-uturn-left" wire:navigate>{{ __('إلغاء الأمر') }}</flux:button>
