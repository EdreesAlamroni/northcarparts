...
new class extends Component
{
    ...
    public function submit(): void
    {
        ...
        $this->news->addMedia($validated['image'])->toMediaCollection('image');
    ...
    <flux:modal name="update-image" class="md:w-lg w-sm">
        <form wire:submit="submit" class="space-y-6">
            <div>
                ...
                <flux:text class="mt-2">{{ __('اختر الصورة الجديدة للخبر.') }}</flux:text>
            </div>

            <div class="space-y-6">
                <flux:field>
                    <flux:label @class(['text-red-600' => $errors->has('image')]) for="image" badge="*" required>{{ __('الصورة') }}</flux:label>
                    <flux:input type="file" id="image" wire:model="image" wire:key="news-image-{{ $imageFieldKey }}" class="p-1 border border-zinc-200 rounded-lg" size="sm" accept="image/jpeg,image/png,image/webp" required />
